...
function RouteComponent() {
  ...
  const { data: searchResults, isLoading: isLoadingSearch } = useQuery(
    searchFollowingQueryOptions({ userId, searchTerm: debouncedQuery }),
  );
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6">
      ...
      <div className="flex w-full flex-col">
        <label className="label mb-1 ml-1"></label>
        <label
      ...
      <div
        className={
          ...
          (!searchInput && allFollowing.length === 0)
      ...
      >
        {isLoadingSearch ? (
          ...
          <div className="py-12 text-center">
            <IconUsers className="text-base-content/75 mx-auto mb-3 size-10" />
          ...
          <div className="py-12 text-center">
            <IconUsers className="text-base-content/75 mx-auto mb-3 h-12 w-12" />
            <p className="text-base-content">No followers yet</p>
          </div>
        ) : (
          <>
            <div className="divide-y divide-slate-100">
              {(searchInput ? (searchResults?.users ?? []) : allFollowing).map(
                (follower) => (
